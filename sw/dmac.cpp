...
public:
    ...
    void readTransferFromFpgaStatDone(uint32_t a, uint64_t b, uint64_t c, uint64_t d) {
        fprintf(stderr, "Channel %d  Bytes: %ld  Sops: %ld  Eops: %ld\n", a, b, c, d);
    }
};
...
int DmaChannel::transferToFpga ( DmaBuffer *buffer, const uint32_t base, const uint32_t bytes, const uint8_t tag )
{
    if (!singleThreadedAccess) pthread_mutex_lock(&channel_lock);
    int v = dmaRequest->transferToFpga(buffer->reference(), base, bytes, tag);
    if (!singleThreadedAccess) pthread_mutex_unlock(&channel_lock);
    return v;
}
