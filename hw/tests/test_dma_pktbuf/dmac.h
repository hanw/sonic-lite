...
public:
    ...
    void setLen(int len) {
       if (len >= size - 16)
          len = size -16;
        *(uint32_t *) buf = len;
        this->len = 16 + ((len % 16) == 0 ? len : (((len >> 4) +1) << 4));
    }
...
public:
    ...
    int transferToFpga (DmaBuffer *buffer, const uint32_t base, const uint32_t bytes, const uint8_t tag );
    // issue a DMA transferFromFpga request
    //int transferFromFpga (DmaBuffer *buffer, const uint32_t base, const uint32_t bytes, const uint8_t tag );
